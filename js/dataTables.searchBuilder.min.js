...
a.s.topGroup.setupLogic();a._setEmptyListener();var b=a.s.topGroup.count();a._updateTitle(b);a._filterChanged(b);a.s.dt.state.save()});q(this.s.topGroup.dom.container).unbind("dtsb-redrawLogic");q(this.s.topGroup.dom.container).on("dtsb-redrawLogic",function(){a.s.topGroup.redrawLogic();var b=a.s.topGroup.count();a._updateTitle(b);a._filterChanged(b)});q(this.s.topGroup.dom.container).unbind("dtsb-add");q(this.s.topGroup.dom.container).on("dtsb-add",function(){var b=a.s.topGroup.count();a._updateTitle(b);
a._filterChanged(b)});q(this.s.dt).on("postEdit postCreate postRemove",function(){a.s.topGroup.redrawContents()});q(this.s.topGroup.dom.container).unbind("dtsb-clearContents");q(this.s.topGroup.dom.container).on("dtsb-clearContents",function(){a._setUp(!1);a._filterChanged(0);a.s.dt.draw()});q(this.s.topGroup.dom.container).on("dtsb-updateTitle",function(){var b=a.s.topGroup.count();a._updateTitle(b);a._filterChanged(b)})};c.prototype._setEmptyListener=function(){var a=this;q(this.s.topGroup.dom.add).on("click",
...
e.defaults.searchBuilder;return(new I(g,h)).getNode()}n(c);m(c);l(c);var e=c.fn.dataTable;c.fn.dataTable.SearchBuilder=I;c.fn.DataTable.SearchBuilder=I;c.fn.dataTable.Group=G;c.fn.DataTable.Group=G;c.fn.dataTable.Criteria=t;c.fn.DataTable.Criteria=t;a=c.fn.dataTable.Api.register;c.fn.dataTable.ext.searchBuilder={conditions:{}};c.fn.dataTable.ext.buttons.searchBuilder={action:function(g,h,p,u){g.stopPropagation();this.popover(u._searchBuilder.getNode(),{align:"dt-container"});void 0!==u._searchBuilder.s.topGroup&&
u._searchBuilder.s.topGroup.dom.container.trigger("dtsb-redrawContents")},config:{},init:function(g,h,p){var u=new c.fn.dataTable.SearchBuilder(g,c.extend({filterChanged:function(y,C){g.button(h).text(C)}},p.config));g.button(h).text(p.text||g.i18n("searchBuilder.button",u.c.i18n.button,0));p._searchBuilder=u},text:null};a("searchBuilder.getDetails()",function(){var g=this.context[0];return g._searchBuilder?g._searchBuilder.getDetails():null});a("searchBuilder.rebuild()",function(g){var h=this.context[0];
