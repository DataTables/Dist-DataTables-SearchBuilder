...
if(k.indexOf(g)===-1){k.push(g);l.push(h);c!==null&&Array.isArray(c[0])&&(c[0]=c[0].sort().join(","));if(c!==null&&h.val()===c[0]){h.attr("selected",true);f(i).removeClass(d.classes.italic)}}};if(e)for(var r=0;r<p.length;r++)q(p[r],o[r]);else q(p,o)}l.sort(function(b,c){if("string"===a.s.type||"num"===a.s.type||"html"===a.s.type||"html-num"===a.s.type)return f(b).val()<f(c).val()?-1:f(b).val()<f(c).val()?1:0;if("num-fmt"===a.s.type||"html-num-fmt"===a.s.type)return+f(b).val().replace(/[^0-9.]/g,"")<
...
b.button(c).text(d.text||b.i18n("searchBuilder.button",e.c.i18n.button,0));d._searchBuilder=e},text:null};b("searchBuilder.getDetails()",function(){var a=this.context[0];if(void 0!==a._searchBuilder)return a._searchBuilder.getDetails()});b("searchBuilder.rebuild()",function(a){var b=this.context[0];if(void 0!==b._searchBuilder)return b._searchBuilder.rebuild(a),this});b("searchBuilder.container()",function(){var a=this.context[0];if(void 0!==a._searchBuilder)return a._searchBuilder.getNode()});a(c).on("preInit.dt.dtsp",
